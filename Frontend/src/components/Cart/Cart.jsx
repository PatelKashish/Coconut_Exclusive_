...
const Cartdisplay = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8000/api/cart").then(res => {
      if (res.ok) {
        return res.json()
      }
  ...
  return (
    <>
      ...
      <Container style={{ top: '131px', left: '186px', width: '1120px', height: '796px', position: 'absolute' }}>
        <h2 style={{ fontSize: '45px', color: '#284B75' }} > Your Cart </h2>
        <Row>
          {Data.map((item) => (
            <Col md={4}>
              <CartCard item={item} />
      ...
      <Link to='/end'>
        <button style={{ left: '1000px', color: '#3C6E71', bottom: '150px', position: 'absolute', fontWeight: 'bold', fontSize: '20px', backgroundColor: 'blue' }} className="b2">
          Place Order
        </button>
      </Link>
